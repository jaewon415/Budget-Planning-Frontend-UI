...
def commitEdittedBudgetPlanOperator(savedBudgetPlan: List[dict], 
    ...
    # budget_graph.createBudgetInformationNode(version, plan_name, plan_created, plan_base_period, start_plan_date, end_plan_date, plan_description, plan_constraints, division, budgetNameEdit, budgetNameView)
    ...
    #     identifier = 'budget' + str(hierarchy_depth)
    #     name = hierarchy[-1]
    #     budget_graph.createBudgetPlanNodes(identifier, name, attributes)
...
def cloneBudgetOperator(savedBudgetPlan: List[dict], 
    ...
    # x = budget_graph.createCheckCount()
    # currentStatusOfNode = x['count']
    # if currentStatusOfNode > 0:
    #     print('here')
    #     budget_graph.setVersionNode(str(savedBudgetPlan), version)
    #     budget_graph.createBudgetInformationNode(version, plan_name, plan_created, plan_base_period, start_plan_date, end_plan_date, plan_description, plan_constraints, division, budgetNameEdit, budgetNameView)
...
# def getCloneData():
#     budget_graph = neo4j_python.Graph(bolt_address, "neo4j", neo4j_passwd)
#     clone_data = budget_graph.getCloneBudgetGraphData()
#     return clone_data

# def cloneOperator(version):
#     # [{'2015': 15897600, '2016': 0, '2017': 0, 'hierarchy': ['데이터사이언스대학원', '과제 A', '간접비', '간접비'], 'id': 0}]
#     # print(version)
#     budget_graph = neo4j_python.Graph(bolt_address, "neo4j", neo4j_passwd)
#     node_paths, result1 = budget_graph.cloneBudgetGraphPath(version)
...
#         require_budget['id'] = id
#         id = id + 1
#         result.append(require_budget)

#     infoProperty = result1[0]['info']
#     end_plan_date = infoProperty['end_plan_date']
#     start_plan_date = infoProperty['start_plan_date']
#     plan_base_period = infoProperty['plan_base_period']
...
#     start_plan_date = '2016-08-03'
#     end_plan_date = '2017-01-03'
#     data_created = infoProperty['plan_created']
#     plan_name = infoProperty['plan_name']
#     plan_description = infoProperty['plan_description']
#     property_list = [plan_base_period, start_plan_date, 
#                      end_plan_date, data_created, plan_name, plan_description]
...
#         columns_dict['type'] = 'number'
#         columns_list.append(columns_dict)
#     return result, columns_list, property_list



# def commitOperator(savedBudgetPlan: List[dict], 
#                    plan_name, plan_created, plan_base_period,
#                    start_plan_date, end_plan_date, plan_description, plan_constraints, division, budgetNameEdit, budgetNameView):
#     """
#     This function will grab the budget plan data from the server
#     and forward the data to the graph database.
#     @param savedBudgetPlan: [{date: int, ..., id: int, hierarchy: []}]
...
#     # version = budget_graph.createVersionNode(str(savedBudgetPlan), plan_name, plan_created, plan_base_period, start_plan_date, end_plan_date, plan_description, plan_constraints, division, budgetNameEdit, budgetNameView)
...
#         budgetPlanString = budgetPlanString + ',' + '(:' + identifier + ' ' + attributesOnly + ')'
#         # budgetPlanString = budgetPlanString + ',' + '(:' + identifier + ' ' + str(attributes) + ')'
#         # print(attributes)
#         # print(name)
#         # print(identifier)
#         # budget_graph.createBudgetPlanNodes(identifier, name, attributes)
...
#     budgetPlanString = 'CREATE' + budgetPlanString[1:]
#     # budgetPlanString = 'CREATE' + budgetPlanString[1:] + '; '+ 'match (n) where not (n)--() RETURN count(n) AS count'

#     # print(plan_constraints)  
#     if all([v != 'None' for k,v in plan_constraints[0].items()]):
#         print('can you go?')
#         buildConstraint(budget_graph, plan_constraints)
   
#     # x = budget_graph.createCheckCount()
#     # print(x)
#     # print(budgetPlanString)
#     print(budgetPlanString)
#     budget_graph.createBudgetPlanNodes(budgetPlanString)
#     # time.sleep(5)
#     x = budget_graph.createCheckCount()
#     # budget_graph.removeConstraintTrigger()
#     currentStatusOfNode = x['count']
#     print(currentStatusOfNode)
#     currentStatusOfNode = 0
#     if currentStatusOfNode > 0:
#         print('here')
#         version = budget_graph.createVersionNode(str(savedBudgetPlan), plan_name, plan_created, plan_base_period, start_plan_date, end_plan_date, plan_description, plan_constraints, division, budgetNameEdit, budgetNameView)
#         budget_graph.createBudgetInformationNode(version, plan_name, plan_created, plan_base_period, start_plan_date, end_plan_date, plan_description, plan_constraints, division, budgetNameEdit, budgetNameView)
...
# def buildConstraint(budget_graph, plan_constraints):
#     # budget_graph = neo4j_python.Graph(bolt_address, "neo4j", neo4j_passwd)
#     count = 1
#     for i in plan_constraints:
#         hierarchyOne = i['hierarchyOne'] 
#         inequality = i['inequality']
#         values = i['values']
#         operator = i['operator'] 
#         hierarchyTwo = i['hierarchyTwo']

#         if inequality == '<=':
#             query = "call apoc.trigger.add(\'wassup" + str(count) + "\'" + \
#             "," + "\"UNWIND $createdNodes AS q MATCH (p) WHERE p.name = " + \
#                 "\'" + str(hierarchyOne) + "\'" + " MATCH (p1) WHERE p1.name = " + \
#                     "\'" + str(hierarchyTwo) + "\'" + " WITH [x IN keys(p) WHERE x <> 'name'] AS props, p, [x IN keys(p1) WHERE x <> 'name'] AS props1, p1 WITH apoc.coll.sum(reduce(accumulator = [], x IN props | accumulator + p[x]))" + str(operator) + str(values) + " <= apoc.coll.sum(reduce(accumulator = [], x IN props1 | accumulator + p1[x])) AS boo WITH DISTINCT boo " + \
#                     "CALL apoc.do.when(NOT boo, 'match (q) where not (q)--() delete (q)') YIELD value RETURN value\", {phase:'afterAsync'})"
#         elif inequality == '=':
#             query = "call apoc.trigger.add(\'wassup" + str(count) + "\'" + \
#             "," + "\"UNWIND $createdNodes AS q MATCH (p) WHERE p.name = " + \
#                 "\'" + str(hierarchyOne) + "\'" + " MATCH (p1) WHERE p1.name = " + \
...
#         count = count + 1
#         if query != '':
#             budget_graph.createConstraintTrigger(query)